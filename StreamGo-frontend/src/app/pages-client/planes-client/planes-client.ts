...
export class PlanesClient implements OnInit {
  ...
  planes: any[] = []; miSuscripcion: any = null; planSeleccionado: any = null; pagoRespuesta: any = null; metodoPago = 'YAPE'; horasPersonalizadas = 24; planPersonalizado: any = null; cargando = false; pagando = false; cargandoPersonalizado = false; mensaje = ''; error = '';
  ngOnInit(): void { this.cargarPlanes(); this.cargarMiSuscripcion(); }
  cargarPlanes(): void { this.cargando = true; this.planesService.listarPlanes().subscribe({ next: data => { this.planes = Array.isArray(data) ? data.filter(plan => plan.activo !== false) : []; this.cargando = false; this.cdr.detectChanges(); }, error: err => { console.error('Error cargando planes:', err); this.planes = []; this.cargando = false; this.error = 'No se pudieron cargar los planes.'; this.cdr.detectChanges(); } }); }
  ...
  calcularPersonalizado(): void { if (!this.horasPersonalizadas || this.horasPersonalizadas <= 0) { this.error = 'Ingresa una cantidad de horas válida.'; return; } this.cargandoPersonalizado = true; this.planPersonalizado = null; this.planesService.obtenerPlanPersonalizado(this.horasPersonalizadas).subscribe({ next: data => { this.planPersonalizado = data; this.cargandoPersonalizado = false; this.cdr.detectChanges(); }, error: err => { console.error(err); this.error = 'No se pudo calcular el plan personalizado.'; this.cargandoPersonalizado = false; this.cdr.detectChanges(); } }); }

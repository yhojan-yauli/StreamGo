...
export class NoticiasClient implements OnInit {
  ...
  noticias: any[] = []; cargando = false; publicando = false; reaccionandoId: number | null = null; mostrarFormulario = false; mensaje = ''; error = ''; noticiaForm: any = { titulo: '', contenido: '', trailer: '' };
  ...
  publicarNoticia(): void { if (!this.noticiaForm.titulo.trim() || !this.noticiaForm.contenido.trim()) { this.error = 'Completa el título y el contenido.'; return; } const usuario = this.auth.getUser(); const idUsuario = usuario?.id || usuario?.userId || usuario?.idUsuario || usuario?.sub || 1; const request = { idUsuario: Number(idUsuario) || 1, idAutor: Number(idUsuario) || 1, titulo: this.noticiaForm.titulo.trim(), contenido: this.noticiaForm.contenido.trim(), trailer: this.noticiaForm.trailer.trim(), reacciones: 0 }; this.publicando = true; this.noticiasService.crear(request).subscribe({ next: () => { this.publicando = false; this.mostrarFormulario = false; this.mensaje = 'Noticia publicada correctamente.'; this.limpiarFormulario(); this.cargarNoticias(); this.cdr.detectChanges(); }, error: err => { console.error(err); this.publicando = false; this.error = 'No se pudo publicar la noticia.'; this.cdr.detectChanges(); } }); }
  reaccionar(noticia: any): void { if (!noticia?.idPost) return; this.reaccionandoId = noticia.idPost; this.noticiasService.reaccionar(noticia.idPost).subscribe({ next: data => { const index = this.noticias.findIndex(item => item.idPost === noticia.idPost); if (index >= 0) { this.noticias[index] = data; this.noticias = [...this.noticias]; } this.reaccionandoId = null; this.cdr.detectChanges(); }, error: err => { console.error(err); this.reaccionandoId = null; this.error = 'No se pudo registrar la reacción.'; this.cdr.detectChanges(); } }); }
  abrirTrailer(url: string): void { if (!url) { this.error = 'Esta noticia no tiene tráiler.'; return; } window.open(url, '_blank'); }
  compartir(noticia: any): void { const texto = `${noticia.titulo} - StreamGO`; const url = window.location.href; if (navigator.share) { navigator.share({ title: noticia.titulo, text: texto, url }); return; } navigator.clipboard.writeText(`${texto} ${url}`); this.mensaje = 'Link copiado para compartir.'; this.cdr.detectChanges(); }
  limpiarFormulario(): void { this.noticiaForm = { titulo: '', contenido: '', trailer: '' }; }
  autor(noticia: any): string { return noticia?.autorNombre || noticia?.usuarioNombre || 'Comunidad StreamGO'; }
}
